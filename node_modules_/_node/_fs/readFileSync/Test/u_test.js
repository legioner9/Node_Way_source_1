...
// fn.module = module;
const readFileSync = ini_fs.s_readFileSync;

readFileSync.exit = true;
readFileSync.utest = true;
readFileSync.fname = 'isFile';

const arr = Fs.readdirSync ( __dirname );

const arr_res = arr.map ( item => {
    const is = readFileSync ( Path.join ( __dirname, item ) );
    return is;
} );

debugger
const u1 = readFileSync ( Path.join ( __dirname, 'error', 'any' ) );
